...
- **`users`** — `id` UUID PK (auto `gen_random_uuid()`), `first_name`, `last_name`, `username`, `email`, `password`. Note: `password` is stored here rather than delegated to `auth.users` — this is a demo pattern; production use should link to `auth.users(id)` instead.
- **`tasks`** — `id` UUID PK, `title`, `description` (nullable), `completed` (bool, default false), `created_at` (default `now()`), `completed_at` (nullable, set when completed), `user_id` FK → `users(id)`.
